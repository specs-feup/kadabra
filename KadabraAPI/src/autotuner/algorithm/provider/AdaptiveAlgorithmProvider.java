 ...
 * @param <K>
 */
public class AdaptiveAlgorithmProvider<A, K> implements Configurable<K>, AlgorithmProvider<A> {
    private Configuration<K> config;
    private Function<K, A> algorithmGenerator;
    private String id;

    public AdaptiveAlgorithmProvider(Function<K, A> algorithmGenerator, String id, Configuration<K> config) {
        this.id = id;
        this.algorithmGenerator = algorithmGenerator;
        this.config = config;
    }

    public AdaptiveAlgorithmProvider(Function<K, A> algorithmGenerator, String id, Supplier<K> argsProvider) {
        this(algorithmGenerator, id, ConfigFactory.fromSupplier(argsProvider));
    }

    public AdaptiveAlgorithmProvider(Function<K, A> algorithmGenerator, String id, K args) {
    ...
    private Algorithm<A> newAlgorithm(K next) {
        ...
        return new SimpleAlgorithm<>(generatedVersion, id2);
    }
