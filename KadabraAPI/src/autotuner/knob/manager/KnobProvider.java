...
/**
 * Provides a knob based on a specified {@link Configuration}
 * 
 * @author tiago
 *
 * @param <A>
 * @param <K>
 */
public class KnobProvider<K> implements Configurable<K>, AlgorithmProvider<K> {
    private Configuration<K> config;
    private Consumer<K> knobApply;
    private String id;

    public KnobProvider(String id, Consumer<K> knobApply, Configuration<K> config) {
        this.id = id;
    ...
    @Override
    public KnobApplier<K> next() {
        K nextKnob = config.next();
        KnobApplier<K> alg = newKnob(nextKnob);
        return alg;
    }

    private KnobApplier<K> newKnob(K next) {
        return new KnobApplier<>(id, next, knobApply);
    }

    @Override
    public boolean hasNext(Algorithm<K> reference) {
        if (!(reference instanceof KnobApplier)) {
            throw new RuntimeException("Algorithm to be used as reference must always be a AlgorithmWithKnob");
        }
        KnobApplier<K> ref = ((KnobApplier<K>) reference);
    ...
    @Override
    public KnobApplier<K> getFirst() {
        K first = config.getFirst();
        return newKnob(first);
    }
