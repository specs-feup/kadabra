...
public class CtType2AType {
    private static final BiFunctionClassMap<CtType<?>, JWeaver, AType<?>> CONVERTER = new BiFunctionClassMap<>();

    static {
        CONVERTER.put(CtClass.class, JClass::new);
        CONVERTER.put(CtInterface.class, JInterfaceType::new);
        CONVERTER.put(CtType.class, CtType2AType::defaultFactory);
    }

    public static AType<?> defaultFactory(CtType<?> element, JWeaver weaver) {
        return new JType<>(element, weaver);
    }

    // Package protected so only CtElement2JoinPoint can use this method
    public static AType<?> convert(CtType<?> element, JWeaver weaver) {
