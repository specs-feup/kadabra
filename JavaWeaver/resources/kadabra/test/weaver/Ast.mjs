...
var $localVariable = Query.search("method", { name: "setLocalVariable" })
...
var $assignment = Query.search("method", { name: "setAssignment" })
    .search("assignment")
    .first();
...
console.log(
    "Search inclusive: " +
        Query.searchFromInclusive(
            Query.search("method", "call").first(),
            "localVariable"
        ).first().code
);
...
console.log(
    "UnaryExpr node: " +
        KadabraNodes.unaryExpression("-", KadabraNodes.literal("1", "int")).code
);
console.log(
    "BinaryExpr node: " +
        KadabraNodes.binaryExpression(
            "+",
            KadabraNodes.literal("1", "int"),
            KadabraNodes.literal("2", "int")
        ).code
);
