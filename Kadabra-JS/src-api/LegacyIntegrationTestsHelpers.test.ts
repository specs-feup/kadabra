...
export class KadabraWeaverTester {
    private static readonly WORK_FOLDER: string = "java_weaver_output";
    ...
    public async test(
        ...
        try {
            ...
            fs.rmdirSync(datastore.get(JavaTypes.LaraiKeys.OUTPUT_FOLDER));
        }
        ...
        expect(KadabraWeaverTester.normalize(out)).toEqual(
            KadabraWeaverTester.normalize(
                fs
                    .readFileSync(expectedResource, "utf8")
                    .replaceAll(
                        `/**** File '${KadabraWeaverTester.WORK_FOLDER}/`,
                        "/**** File '"
                    )
            )
        );
